...
DOCS = [
    {
        ...
        "menu": dict(
            ...
            functions=[
                ...
                fn("2", "新增", "Add", "新增学年学期开课时间配置。", "工具栏。", "同学期不可重复（正式校验待补）。", "表单/弹窗（原型占位）。"),
            ...
            pending="编辑弹窗文案提示周数（6/17）与原型数据推导（5/14）不一致，正式以校历决议统一；新增正式表单待补全。",
        ),
    },
    {
        "folder": "特殊课程设置",
    ...
    },
    {
        ...
        "meta": {
            ...
            "background": "主路径为「计划 → 安排 → 名单」。计划层负责生成与生效；合班与分组职责分离，生成时禁止自动合班。",
        },
        "menu": dict(
            section_no="2.2.2.1 ",
            ...
            biz_flow="进入【开课计划】→ 生成开课任务 → 修改教学任务 → 必要时合班 → 勾选生效 → 进入【开课安排】。",
            ...
            field_tables=[
                ...
                (
                    ...
                    [
                        ...
                        f("2", "学分/总学时/起止周/选课类型", "—", "只读", "是", "双方须相同", "—", "否", ""),
                        f("3", "专业批次", "Prog. Batch", "穿梭", "是", "至少保留1个", "合入后挂同一教学班；人数预留累加；清空分组教师", "否", ""),
                    ],
                ),
            ],
            functions=[
                fn("1", "生成开课任务", "Generate Offering Tasks", "从执行计划生成计划行与教学班。", "弹窗勾选或一键生成；禁止自动合班。", "已存在课+专业+批次跳过。", "弹窗：生成开课任务。"),
                ...
                fn("4", "生效", "Activate Plan", "批量将计划生效。", "工具栏勾选生效。", "进入开课安排可见。", "无。"),
                fn("5", "删除", "Delete", "删除计划行与关联数据。", "工具栏确认。", "可再生成。", "无。"),
                fn("6", "预置人数", "View Preset Count", "查看可修预置人数。", "列点击。", "—", "学生预置人数抽屉。"),
                fn("7", "操作记录", "Change Log", "查看字段变更。", "列→查看。", "—", "修改记录窗。"),
            ],
    ...
    },
    {
        ...
        "menu": dict(
            ...
            "各步可生效/撤回/退回。生效门槛含分组学时校验与全部教师已确认。",
            ...
            "步骤2含Support；步骤3含安排教师/共同授课；展开子表为学时投递明细。",
            ...
            field_tables=[
                (
                    ...
                    [
                        ...
                        f("7", "共同授课状态", "Shared Teaching", "Y/N", "—", "—", "须先有任课教师", "是", "N"),
                ...
                (
                    ...
                    [
                        ...
                        f("5", "授课教师", "Lecturer", "选择器", "是", "至少1名", "多教师须Coordinator", "否", ""),
            ...
            functions=[
                ...
                fn("5", "授课确认管理", "Teaching Confirmation", "查看/代确认。", "步骤3。", "生效门槛。", "授课确认管理。"),
                fn("6", "共同授课", "Shared Teaching", "配置共同授课课号。", "步骤3。", "须先有任课教师。", "共同授课设置。"),
...
def build_one(spec: dict, *, version: str, date_ymd: str, allow_overwrite: bool = False) -> Path:
    ...
    out = BASE_OUT / parent / spec["folder"] / filename
    ...
    add_para(doc, "本文档为单个二级菜单需求说明；已确认需求标记为「已确认」。不含操作流程图、校选课程管理（本期不输出）。")
...
def _latest_version_num(menu_folder: Path, menu: str) -> int:
    ...
    best = 0
    pat = re.compile(rf"^{re.escape(menu)}\d{{8}}V(\d+)\.docx$")
